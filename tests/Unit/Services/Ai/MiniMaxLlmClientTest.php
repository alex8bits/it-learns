...
/**
 * Locks the MiniMax ChatCompletion v2 wire contract fixed at Stage 4:
 * if these tests start failing against an updated client, the provider
 * API has drifted and only MiniMaxLlmClient needs to change.
 */
class MiniMaxLlmClientTest extends TestCase
{
    private MiniMaxLlmClient $client;
    ...
    protected function setUp(): void
    {
        ...
        $this->client = new MiniMaxLlmClient;
    }
    ...
    public function test_sends_the_request_to_the_chatcompletion_v2_endpoint_with_bearer_auth_and_messages(): void
    {
        ...
        Http::assertSent(function (Request $request): bool {
            return $request->url() === 'https://api.minimax.io/v1/text/chatcompletion_v2'
    ...
    public function test_zero_base_resp_status_code_is_treated_as_success(): void
    {
        Http::fake(['*' => Http::response([
            'base_resp' => ['status_code' => 0, 'status_msg' => ''],
            'choices' => [['message' => ['content' => 'Loops repeat code.']]],
            'usage' => ['total_tokens' => 313],
        ])]);
    ...
    public function test_base_resp_error_inside_http_200_fails_loud_and_is_logged_with_error(): void
    {
        Http::fake(['*' => Http::response([
        ...
        Log::shouldReceive('info')
            ...
            ->with('ai.llm_call', Mockery::on(function (array $context): bool {
                return $context['provider'] === 'minimax'
                    ...
                    && str_contains((string) $context['error'], 'MiniMax API error 1004')
                    && str_contains((string) $context['error'], 'invalid api key')
                    && ! str_contains((string) $context['error'], 'test-key')
                    && ! array_key_exists('system_prompt', $context)
                    && ! array_key_exists('response', $context);
            }));

        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('MiniMax API error 1004');
    ...
    public function test_malformed_200_body_fails_loud_and_is_logged_with_error(string $body): void
    {
        ...
        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('Unexpected MiniMax API response structure');

        $this->client->complete('You are a tutor.', 'Explain loops.');
    }

    /**
     * HTTP 200 bodies that carry no base_resp error and no expected
     * content path.
     *
    ...
    {
        Http::fake(['*' => Http::response(['base_resp' => ['status_code' => 1004]], 500)]);
    ...
    /**
     * MiniMax ChatCompletion v2-shaped response with a configurable
     * usage block (OpenAI-shaped, as fixed by the Stage 4 contract).
     *
