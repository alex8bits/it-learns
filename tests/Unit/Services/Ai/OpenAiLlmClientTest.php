...
class OpenAiLlmClientTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->client = new OpenAiLlmClient;
    }
    ...
    public function test_clients_whitelist_maps_every_entry_to_an_existing_llm_client_implementation(): void
    {
        /** @var array<string, class-string<LlmClient>> $clients */
        $clients = config('ai.clients');

        $this->assertSame(
            ['dummy', 'openai', 'anthropic', 'minimax', 'openai_compatible'],
            array_keys($clients),
        );

        foreach ($clients as $key => $class) {
            $this->assertTrue(class_exists($class), "Whitelist entry [{$key}] points to a missing class [{$class}].");
            $this->assertTrue(
                is_subclass_of($class, LlmClient::class),
                "Whitelist entry [{$key}] does not implement LlmClient.",
            );
        }
    }

    public function test_openai_without_api_key_fails_loud_on_register(): void
    {
        config([
            'ai.provider' => 'openai',
            'ai.api_key' => null,
    ...
    public function test_sends_the_request_to_the_openai_endpoint_with_bearer_auth_and_messages(): void
    {
    ...
    public function test_malformed_200_body_fails_loud_and_is_logged_with_error(string $body): void
    {
        ...
        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('Unexpected OpenAI API response structure');
