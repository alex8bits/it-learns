<?php

declare(strict_types=1);

namespace Tests\Unit\Actions\Admin;

use App\Actions\Admin\ChangeUserRole;
use App\Enums\AdminAuditAction;
use App\Enums\UserRole;
use App\Models\AdminAuditLog;
use App\Models\User;
use App\Services\Admin\AdminAuditLogger;
use Mockery;
use RuntimeException;
use Spatie\Permission\Models\Role;
use Tests\TestCase;

class ChangeUserRoleTest extends TestCase
{
    protected function setUp(): void
    {
        parent::setUp();

        Role::findOrCreate(UserRole::User->value, 'web');
        Role::findOrCreate(UserRole::Admin->value, 'web');
    }

    public function test_syncs_role_and_creates_audit_log(): void
    {
        $admin = User::factory()->admin()->create();
        $target = User::factory()->create();
        $target->assignRole(UserRole::User->value);
        $this->actingAs($admin);

        app(ChangeUserRole::class)->execute($target, UserRole::Admin, $admin);

        $this->assertTrue($target->fresh()->hasRole(UserRole::Admin->value));
        $this->assertFalse($target->fresh()->hasRole(UserRole::User->value));

        $this->assertDatabaseHas('admin_audit_logs', [
            'action' => AdminAuditAction::UserRoleChanged->value,
            'admin_id' => $admin->id,
            'subject_type' => (new User)->getMorphClass(),
            'subject_id' => $target->id,
        ]);
    }

    public function test_meta_contains_old_and_new_roles(): void
    {
        $admin = User::factory()->admin()->create();
        $target = User::factory()->create();
        $target->assignRole(UserRole::User->value);
        $this->actingAs($admin);

        app(ChangeUserRole::class)->execute($target, UserRole::Admin, $admin);

        $log = AdminAuditLog::query()
            ->where('subject_id', $target->id)
            ->firstOrFail();

        $meta = $log->meta;

        $this->assertIsArray($meta);
        $this->assertSame([UserRole::User->value], $meta['old']);
        $this->assertSame(UserRole::Admin->value, $meta['new']);
    }

    public function test_transaction_rollback_on_audit_failure(): void
    {
        $admin = User::factory()->admin()->create();
        $target = User::factory()->create();
        $target->assignRole(UserRole::User->value);

        $mock = Mockery::mock(AdminAuditLogger::class);
        $mock->shouldReceive('log')
            ->once()
            ->andThrow(new RuntimeException('boom'));
        $this->instance(AdminAuditLogger::class, $mock);

        try {
            app(ChangeUserRole::class)->execute($target, UserRole::Admin, $admin);
            $this->fail('Expected RuntimeException to bubble out of the action.');
        } catch (RuntimeException $exception) {
            $this->assertSame('boom', $exception->getMessage());
        }

        $this->assertDatabaseMissing('admin_audit_logs', [
            'subject_id' => $target->id,
        ]);
        $this->assertFalse($target->fresh()->hasRole(UserRole::Admin->value));
        $this->assertTrue($target->fresh()->hasRole(UserRole::User->value));
    }
}
