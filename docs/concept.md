...
- Набор **практических заданий** (выполняются последовательно после
  обязательного минимума теории).
- **Обязательный минимум:** урок завершается по первым по порядку
  опубликованным заданиям каждой части — теория:
  `progress.theory_required_per_lesson` (по умолчанию 3), практика:
  `progress.practice_required_per_lesson` (по умолчанию 1); задания сверх
  минимума опциональны (переходы и кнопки — §4 и §5.3, правило — §6).
...
- **Обязательный минимум теории:** первые `progress.theory_required_per_lesson`
  (по умолчанию 3) опубликованных вопроса по порядку обязательны — на каждый
  нужно дать верный ответ (если вопросов меньше порога — требуется все:
  `min(K, N)`). Вопросы сверх минимума — опциональный резерв: открываются
  кнопкой «Ещё один вопрос», на завершение урока не влияют.
- После достижения минимума теории пользователь сам выбирает: «Перейти к
  практике» (или «Перейти к следующему уроку», если в уроке нет практических
  заданий) либо продолжить резервные вопросы («Ещё один вопрос»).
  Автоперехода к практике нет.
...
### 5.3. Проверка
- **Совпало с ожидаемым** → задание пройдено (`Passed`); в пределах
  обязательного минимума открывается следующее.
- **Не совпало / ошибка исполнения** → блокировать переход к следующему заданию до получения корректного результата.
- **Обязательный минимум практики:** `progress.practice_required_per_lesson`
  (по умолчанию 1) первых по порядку опубликованных заданий обязательны —
  каждое должно получить `Passed` (если заданий меньше порога — требуется
  все: `min(K, N)`). Остальные задания — по желанию: открываются кнопкой
  «Ещё одно задание», на завершение урока не влияют.
- Практика в уроке открывается после достижения обязательного минимума
  теории (до этого таб практики закрыт — «Доступна после обязательных
  вопросов»).
- После минимума практики пользователь выбирает: «Перейти к следующему
  уроку» либо продолжить резервные задания («Ещё одно задание»). Если
  текущий урок — последний в курсе (`nextLesson === null`), показывается
  кнопка «К списку уроков курса», ведущая на страницу курса.
  Автоперехода нет.
...
- **Сравнение результатов с эталоном:**
  - **Модель — хеш-эталон.** Автор задания при создании указывает **эталонный набор строк** (JSON, каноническая сериализация: **порядок строк значим** — эталон задаёт порядок строк; нормализуются порядок колонок по списку ожидаемых, строки `trim`+`lower`, числа как строки с фиксированной точностью, SQL NULL — сентинелом). Платформа вычисляет **хеш эталона** при сохранении задания.
...
Правила (реализовано, Этапы 7–8; правило завершения уточнено 2026-09-19 —
«обязательный минимум»):

- Статусы — enum'ы `CourseProgressStatus` и `LessonProgressStatus` (обе:
  `InProgress` | `Completed`); «не начат» — отсутствие записи.
- **Правило завершения урока** («первые K») инкапсулировано в одном месте —
  `App\Services\Progress\LessonCompletionChecker`: урок считается
  пройденным, когда пользователь **верно ответил на первые K по порядку
  опубликованных теоретических задач** и **получил `Passed`-сабмит на первые
  M по порядку опубликованных практических задач**. Пороги K и M —
  `config/progress.php`: `progress.theory_required_per_lesson` (по
  умолчанию 3; env `PROGRESS_THEORY_REQUIRED_PER_LESSON`) и
  `progress.practice_required_per_lesson` (по умолчанию 1; env
  `PROGRESS_PRACTICE_REQUIRED_PER_LESSON`). Если опубликованных задач меньше
  порога — требуется все (`min(K, N)`); пустое обязательное окно части —
  «пусто = пройдено» (vacuously done). Задачи за пределами обязательного
  окна опциональны и на завершение не влияют; повторный неверный ответ на
  обязательный вопрос после Completed возвращает урок в `InProgress`,
  неверный ответ на опциональный вопрос статус не понижает.
