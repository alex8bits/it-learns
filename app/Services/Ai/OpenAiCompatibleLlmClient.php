...
/**
 * OpenAI-compatible implementation of LlmClient: the OpenAI wire
 * protocol (POST {base_url}/chat/completions, Bearer auth, OpenAI-shaped
 * messages) pointed at a self-hosted or proxied endpoint — Ollama, vLLM,
 * LiteLLM proxies and the like. The base_url comes from config('ai.base_url')
 * and is validated on boot by AiConfigValidator (public URLs only —
 * SSRF guard). Response contract: choices[0].message.content and
 * usage.total_tokens; when usage is missing, the shared strlen-based
...
class OpenAiCompatibleLlmClient implements LlmClient
{
    ...
    /**
     * Send the completion request to the configured compatible endpoint.
     *
    ...
    public function complete(string $systemPrompt, string $userMessage, array $options = []): LlmResponse
    {
        ...
        $endpoint = rtrim((string) config('ai.base_url'), '/').'/chat/completions';
    ...
    private function extractContent(mixed $json): string
    {
        ...
        if (! is_string($content)) {
            throw new RuntimeException('Unexpected OpenAI-compatible API response structure: missing choices[0].message.content');
        }
