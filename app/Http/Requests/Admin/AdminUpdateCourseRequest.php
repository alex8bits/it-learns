...
class AdminUpdateCourseRequest extends FormRequest
{
     ...
     * `$this->authorize('update', $course)` so the ability resolves
    ...
    /**
     * Same shape as the create request minus `ai_course_prompt`: the
     * course AI prompt is edited only through its dedicated endpoint
     * (CoursePromptController), so the update payload ignores it. The
     * preview file stays nullable so an update without a new image
     ...
     * `UpdateCourse`; the uploaded file is fetched separately via
     * `$request->file('preview_image')`.
     *
     * @return array{title: string, description: string, status: string, sort_order?: int, preview_image?: mixed}
     */
