...
const props = defineProps({
    ...
    // back link can point at the lesson edit page. `runtimeOptions` comes
    // from PracticeRuntime::options() — no enum duplicates in JS.
...
const submit = () => {
    // An empty order is dropped entirely so the backend appends the task
    // to the end of the lesson instead of casting an explicit null to 0.
    form.transform((data) => {
        const payload = { ...data };
        ...
        return payload;
    }).post(`/admin/lessons/${props.lesson.id}/practice-tasks`);
};

// Check-the-reference-query block: local state only, deliberately NOT in
// useForm — it never travels to the store payload. Plain fetch (no axios
...
<template>
    <AdminLayout>
        ...
        <h1 class="text-2xl font-semibold text-gray-900 mb-2">Новое практическое задание</h1>
        <p class="text-sm text-gray-500 mb-6">Урок: {{ lesson.title }}</p>
        <form
        ...
        >
            ...
            />
            <p class="-mt-2 mb-3 text-xs text-gray-500">
                Оставьте пустым — задание будет добавлено в конец урока
            </p>
            <label class="block mb-3">
            ...
            <div class="mt-4">
                <Button type="submit" :processing="form.processing">Создать задание</Button>
            </div>
        </form>
